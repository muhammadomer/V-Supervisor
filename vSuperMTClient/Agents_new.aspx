    ...
    <script>
        ...
        function LoadAgentStateFilterList() {
            ...
            for (var i = 0; i < AgentStatesFiltersList.length; i++) {
                ...
                //if ($.inArray(AgentState, UncheckedAgentStates) > -1 || $.inArray(AgentState, CheckedAgentStates2) == -1) {
                //    //|| $.inArray(AgentState,CheckedAgentStates2) == -1
                //    $Cb.prop("checked", false);
                //}
                //else {
                //    $Cb.prop("checked", true);
                //    CheckedAgentStates.push(AgentState);
                //    CheckedAgentStates1.push(AgentState);
                  

                //}
                //if ($Cb.is(':checked')) {
                //    UncheckedAgentStates.splice($.inArray($Cb.val(), UncheckedAgentStates), 1);
                //    CheckedAgentStates.push($Cb.val());
                //    CheckedAgentStates1.push($Cb.val());


                //    LoadAgentsPartialInfo();
                //} else {
                //    UncheckedAgentStates.push($Cb.val());
                //    CheckedAgentStates.splice($.inArray($Cb.val(), CheckedAgentStates), 1);
                //    CheckedAgentStates1.splice($.inArray($Cb.val(), CheckedAgentStates1), 1);
                ...
                $Cb.on('ifChecked', function () {
                    ...
                    CheckedAgentStates1.push($(this).val());
                ...
                $Cb.on('ifUnchecked', function () {
                    ...
                    CheckedAgentStates1.splice($.inArray($(this).val(), CheckedAgentStates1), 1);
        ...
        function LoadQueuesList() {
            ...
            for (var i = 0; i < AllQueues.length; i++) {
                ...
                $label = $("<label/>");
                var $Cb = $('<input />', { "id": "cb_" + QueueName + "", "type": 'checkbox', "class": "icheck", "checked": "checked", "value": "" + QueueName + "" }).appendTo($label);
                ($label).appendTo($div);
...
function secondsTimeSpanToHMS(s) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data != null) {
                       ...
                       //     $('#cb_Avatar').attr('checked', false);
                       // }
                       // else {
                         //   $('#cb_Avatar').attr('checked', false);
                       // }
                       // $('#cb_Avatar').val(data.d.DisplayAvatar)
                    }
                }                
            });
        }
     ...
     // var  SortBy2 = $('#lstAgentSort').val();


       // var s = SortBy1.text;


     // var  nooftiles2 = $("#ddlNoOfTiles").val();

       // let SortBy2 = SortBy1;
        // $('#lstAgentSort option:selected').val();
        console.log(SortBy1 + " abcd");

      //  SortBy1 = $('#lstAgentSort').val();

      //  var Param = { "UserId": 1, "SortBy": SortBy1, "QueueList": CheckedQueues1, "AgentState": agid, "NumberOfAgents": nooftiles1, "DisplayAvatar": 6 };
        ...
        function saveAgentDisplay() {
            ...
            $('#AgentStateFiltersList input').each(function (index) {
                if ($(this).is(':checked')) {
                    testAgents.push($(this).val());
                }
            });
            console.log(testAgents);
            SortBy1 = $('#lstAgentSort').val();

            var Param = { "UserId": 1, "SortBy": SortBy1, "QueueList": CheckedQueues1, "AgentState": testAgents, "NumberOfAgents": nooftiles1, "DisplayAvatar": avatar };
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.d === "1") {
                        debugger;
                        $().toastmessage("showSuccessToast", "Agent updated sucessfully.");
